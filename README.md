...
* **style**: Determens the appearenc of the appointment booker. When set to `fullscreen` the boooker will appear in fullsreen mode, in `modal-view` mode the booker will appear as a modal with the page dimmed in the backround. In `embedded` mode the content of the booker will appear embeded in the page, and in `embeddedSafe` mode the booker will be embeded in the page but wont use the style sheet of the page. When in `embeddedSafe` mode you can also declacer a `parentElement` attribute, and the booker will embed itself inside the given `parentElement`.
